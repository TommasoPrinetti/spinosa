...
Your question
    ↓
Searcher finds evidence → writes evidence_packet_{session_id}.md
    ↓
...
| `evidence_packet_{session_id}.md` | Searcher | `evidence_packet_20260629-abcd1234.md` |
| `analysis_{session_id}.md` | Analyst | `analysis_20260629-abcd1234.md` |
| `serendipity_{session_id}.md` | Serendippo | `serendipity_20260629-abcd1234.md` |
| `e_{session_id}.md` | Evaluator | `e_20260629-abcd1234.md` |
| Terminal `NN_*.md` | Verifier | In-place (`pass` / `pass_with_corrections` on report YAML) |
...
Legacy `evidence_packet.md` (no suffix) may exist from older routes or startup; steady-state routes use session-scoped names to avoid parallel-route collisions.

## Delivering answers to the user

Non-fast-path routes produce a numbered report (`NN_descriptive-name.md`). After verifier and evaluator complete, tell the user **where the file is** — do not paste the full report into chat.

Example deliver message:

> Done. Verified report: `agent_reports/02_coastal-erosion.md` (pass_with_corrections). Evaluator: no_edit.
