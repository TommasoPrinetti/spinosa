...
export function header(credentials?: Credentials) {
  const password = credentials?.password ?? process.env.SPINOSA_SERVER_PASSWORD
  if (!password) return undefined

  return `Basic ${Buffer.from(`${credentials?.username ?? process.env.SPINOSA_SERVER_USERNAME ?? "spinosa"}:${password}`).toString("base64")}`
}
