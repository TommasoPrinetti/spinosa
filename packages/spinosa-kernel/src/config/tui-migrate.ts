...
const TUI_SCHEMA_URL = "https://spinosa.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from spinosa.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const spinosa = await spinosaFiles(input)
  for (const file of spinosa) {
...
async function spinosaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "spinosa"),
    ...(await Filesystem.findUp(["spinosa.json", "spinosa.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "spinosa"))
  }
  if (Flag.SPINOSA_CONFIG) files.push(Flag.SPINOSA_CONFIG)
