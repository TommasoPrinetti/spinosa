...
import { SkillPlugin } from "@spinosa/kernel-core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@spinosa/kernel-core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@spinosa/kernel-core/v1/config/error"
...
const SPINOSA_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with opencode. The model's intuition for what an
// spinosa.json should look like is often wrong, and opencode hard-fails on
...
const CUSTOMIZE_SPINOSA_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating spinosa's own configuration: spinosa.json, spinosa.jsonc, files under .spinosa/, or files under ~/.config/spinosa/. Also use when creating or fixing spinosa agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring spinosa itself."
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, SPINOSA_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@spinosa/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_SPINOSA_SKILL_NAME] = {
          name: CUSTOMIZE_SPINOSA_SKILL_NAME,
          description: CUSTOMIZE_SPINOSA_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_SPINOSA_SKILL_BODY,
        }
