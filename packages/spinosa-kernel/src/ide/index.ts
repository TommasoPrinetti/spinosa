...
import { IdeEvent } from "@spinosa/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["SPINOSA_CALLER"] === "vscode" || process.env["SPINOSA_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.spinosa"], {
    nothrow: true,
  })
