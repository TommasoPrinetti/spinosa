...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in spinosa.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .spinosa/ subdirectory too)
  const candidates = [path.join(baseDir, "spinosa.json"), path.join(baseDir, "spinosa.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".spinosa", "spinosa.json"), path.join(baseDir, ".spinosa", "spinosa.jsonc"))
  }
  ...
  // Default to spinosa.json if none exist
  return candidates[0]
}
