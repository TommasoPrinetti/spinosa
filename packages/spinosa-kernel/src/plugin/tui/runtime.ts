...
import { hasTheme, upsertTheme } from "@spinosa/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@spinosa/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@spinosa/kernel-core/v1/config/plugin"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@spinosa/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".spinosa"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".spinosa", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".spinosa", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SPINOSA_PURE ? [] : pluginOrigins
    if (Flag.SPINOSA_PURE && pluginOrigins.length) {
    }
