...
- Author package entrypoint is `@spinosa/plugin/tui`.
...
- Import types from `@spinosa/plugin/tui`.
- `@spinosa/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Spinosa version.
...
When `plugin` entries exist in a writable `.opencode` dir or `SPINOSA_CONFIG_DIR`, Spinosa installs `@spinosa/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@spinosa/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Spinosa's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Spinosa registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@spinosa/plugin/tui`.
...
- `--pure` / `SPINOSA_PURE` skips external TUI plugins only.
