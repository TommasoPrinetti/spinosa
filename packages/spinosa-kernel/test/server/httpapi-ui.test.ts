  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("SPINOSA_SERVER_PASSWORD", original.envPassword)
        restoreEnv("SPINOSA_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            SPINOSA_SERVER_PASSWORD: input?.password,
            SPINOSA_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("does not proxy a remote UI when the embedded UI is unavailable", () =>
    Effect.gen(function* () {
      let requested = false

      const response = yield* uiApp({
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("remote"), () => {
          requested = true
        }),
      }).request("/")
  ...
  it.live("does not proxy non-HTML assets when the embedded UI is unavailable", () =>
    Effect.gen(function* () {
      let requested = false
      const response = yield* uiApp({
        disableEmbeddedWebUi: true,
  ...
  it.live("does not proxy request bodies when the embedded UI is unavailable", () =>
    Effect.gen(function* () {
      let requested = false
      const response = yield* uiApp({
        disableEmbeddedWebUi: true,
  ...
  it.live("rejects reusable query credentials for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        password: "secret",
        username: "opencode",
