...
`,
  ...
  const pure = process.env.SPINOSA_PURE
  const meta = process.env.SPINOSA_PLUGIN_META_FILE
  process.env.SPINOSA_PURE = "1"
  process.env.SPINOSA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SPINOSA_PURE
    } else {
      process.env.SPINOSA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SPINOSA_PLUGIN_META_FILE
    } else {
      process.env.SPINOSA_PLUGIN_META_FILE = meta
    }
  }
})
