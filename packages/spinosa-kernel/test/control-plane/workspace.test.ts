...
import { EventSequenceTable } from "@spinosa/kernel-core/event/sql"
...
const originalEnv = {
  SPINOSA_AUTH_CONTENT: process.env.SPINOSA_AUTH_CONTENT,
  SPINOSA_EXPERIMENTAL_WORKSPACES: process.env.SPINOSA_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SPINOSA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SPINOSA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SPINOSA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SPINOSA_EXPERIMENTAL_WORKSPACES).toBe("true")
