# Spinosa V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@spinosa/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an Spinosa extension point.
...
```ts
import { define } from "@spinosa/plugin/v2/promise"
