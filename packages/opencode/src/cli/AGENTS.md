# OpenCode CLI Layer (V1 yargs)

`packages/opencode/src/cli/` is the legacy yargs command surface for the shipped `opencode` binary. It hosts TUI **adapters**, not the canonical UI (that is `@opencode-ai/tui`).
...
| Trigger | File | Flow |
| ------- | ---- | ---- |
| `opencode` (default) | `cmd/tui.ts` | → `cli/tui/layer.ts` → `@opencode-ai/tui` `run()` |
...
Mini TUI lives in `cmd/run/` — **not** the full `packages/tui` app tree. It reuses TUI primitives (`@opencode-ai/tui/keymap`, editor, spinner, config, theme).
...
Compatibility re-exports (prefer `@opencode-ai/tui` subpaths for new code):

- `src/cli/logo.ts` → `@opencode-ai/tui/logo`
- `src/util/locale.ts`, `error.ts`, `parsers-config.ts` → `@opencode-ai/tui/util/*`
...
`packages/cli` (`lildax`) is the Effect-based preview CLI. Default command also launches `@opencode-ai/tui` but through daemon transport — see `packages/cli/AGENTS.md`.
