...
> current private `@spinosa/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@spinosa/ai`
...
- Secondary audience: Spinosa and other durable agent runtimes
...
import { LLM } from "@spinosa/ai"
import { OpenAI } from "@spinosa/ai/providers/openai"
...
```ts
import { Message } from "@spinosa/ai"
...
import { LLM, Tool } from "@spinosa/ai"
...
Spinosa and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@spinosa/ai/promise"
import { OpenAI } from "@spinosa/ai/providers/openai"
...
```ts
import { LLMSchema } from "@spinosa/ai/schema"
...
```ts
import { Provider, Protocol } from "@spinosa/ai/provider"
...
@spinosa/ai/providers/openai
@spinosa/ai/providers/anthropic
@spinosa/ai/providers/google
...
```
...
Spinosa should migrate to `generateTurn` / `streamTurn`, preserving its durable
