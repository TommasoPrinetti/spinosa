import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@spinosa/plugin/tui"
...
function routeNavigate(route: ReturnType<typeof useRoute>, name: string, params?: Record<string, unknown>) {
  ...
  if (name === "session") {
    ...
    route.navigate({ type: "workspace", sessionID })
    return
  }
...
function routeCurrent(route: ReturnType<typeof useRoute>): TuiPluginApi["route"]["current"] {
  ...
  if (route.data.type !== "plugin") return { name: "home" }
