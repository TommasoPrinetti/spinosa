  ...
  if (!servicePromise) {
    servicePromise = (async () => {
      ...
      try {
        ...
        const { PaddleOcrService: OcrService, V4_SERVER_DOC_MODEL } = await import("ppu-paddle-ocr")
        if (needsPolyfill) delete (globalThis as Record<string, unknown>).document
        onLog?.("PPU PaddleOCR: step 2 - import OK, constructing...")
        const instance = new OcrService({ model: V4_SERVER_DOC_MODEL, processing: { engine: "canvas-native" } })
