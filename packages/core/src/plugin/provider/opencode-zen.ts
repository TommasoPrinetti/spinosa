...
import type { CredentialValue } from "@spinosa/sdk/v2/types"
...
export const OpenCodeZenPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "opencode",
  ...
  const load = Effect.fn("OpenCodeZenPlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Spinosa provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
