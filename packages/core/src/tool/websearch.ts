...
export class ConfigService extends Context.Service<ConfigService, Config>()("@spinosa/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SPINOSA_WEBSEARCH_PROVIDER === "exa" || process.env.SPINOSA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SPINOSA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SPINOSA_EXPERIMENTAL") || truthy("SPINOSA_ENABLE_EXA") || truthy("SPINOSA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SPINOSA_ENABLE_PARALLEL") || truthy("SPINOSA_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `spinosa/${InstallationVersion}`,
