...
#
# Stable:  bash framework/bin/publish-release.sh X.Y.Z
# Beta:   bash framework/bin/publish-dev-release.sh X.Y.Z-beta.N
#           (or: bash framework/bin/publish-release.sh X.Y.Z-beta.N --prerelease)

REPLACE_ASSETS=0
PRERELEASE=0

if [[ -z "${1:-}" ]]; then
  echo "Usage: bash framework/bin/publish-release.sh <version> [--prerelease] [--replace-assets]"
  echo "       bash framework/bin/publish-dev-release.sh <version> [--replace-assets]"
  echo ""
  echo "Examples:"
  echo "  bash framework/bin/publish-release.sh 0.7.7"
  echo "  bash framework/bin/publish-dev-release.sh 0.8.0-beta.1"
  exit 1
fi
...
if [[ "$PKG_VERSION" != "$VERSION" ]]; then
  echo "Error: package.json version=${PKG_VERSION:-<missing>} does not match release version ${VERSION}"
  echo "  Bump the version field in package.json before publishing."
  exit 1
fi
...
bash "${REPO_ROOT}/framework/bin/package-release.sh" "$VERSION"
