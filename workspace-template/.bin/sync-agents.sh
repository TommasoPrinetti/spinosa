...
if [[ ! -f "$agents_src" ]]; then
  agents_src="${REPO_ROOT}/workspace-template/AGENTS.md"
  claude_dst="${REPO_ROOT}/workspace-template/CLAUDE.md"
fi
copy_file_or_die "$agents_src" "$claude_dst"
today="$(date +%Y-%m-%d)"
  # Update updated date and add provenance fields in the frontmatter block.
  # Strip any stale provenance fields from previous runs first.
  sed -i.bak \
    -e '/^generated_by:/d' \
    -e '/^generated_at:/d' \
    -e '/^processing_status:/d' \
    -e 's/^updated:.*/updated: '"$today"'/' \
    -e '/^updated:/a\'$'\n''generated_by: sync-agents\'$'\n''generated_at: '"$today"''$'\n''processing_status: auto_generated' \
    "$claude_dst" 2>/dev/null || \
  sed -i '' \
    -e '/^generated_by:/d' \
    -e '/^generated_at:/d' \
    -e '/^processing_status:/d' \
