...
rapidocr_ocr_available() {
  ...
  local framework_bin="${TEMPLATE_ROOT:-.}/.bin/lib/vendor/spinosa-${PLATFORM}/rapidocr-cli"
...
markitdown_available() {
  ...
  local framework_bin="${TEMPLATE_ROOT:-.}/.bin/lib/vendor/spinosa-${PLATFORM}/markitdown-cli"
...
markitdown_script_path() {
  local script="${TEMPLATE_ROOT:-.}/.bin/lib/markitdown-cli.py"
  [[ -f "$script" ]] || return 1
  echo "$script"
}
...
installed_release_version() {
  local version
  version="$(framework_version "$TEMPLATE_ROOT")"
  if [[ -n "$version" && "$version" != "dev" ]]; then
    echo "$version"
    return 0
  fi
  if declare -F spinosa_latest_complete_version >/dev/null 2>&1; then
    version="$(spinosa_latest_complete_version)"
    if [[ -n "$version" ]]; then
      echo "$version"
      return 0
    fi
