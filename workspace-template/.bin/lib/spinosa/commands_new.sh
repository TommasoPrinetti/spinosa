...
cmd_new() {
  ...
  if [[ ! -f "${TEMPLATE_ROOT}/.spinosa/workspace-files.tsv" ]]; then
    die "Framework manifest not found: ${TEMPLATE_ROOT}/.spinosa/workspace-files.tsv"
  fi

  mkdir -p "$workspace_path/.spinosa"

  local source_framework_version
  source_framework_version="$(framework_version "$TEMPLATE_ROOT")"
...
done < "${TEMPLATE_ROOT}/.spinosa/workspace-files.tsv"
