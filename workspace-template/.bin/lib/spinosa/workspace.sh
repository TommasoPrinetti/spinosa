...
CONFIG_EOF
  ...
  mkdir -p "$root/NO_MATCHsnippets"
  cat > "$root/NO_MATCHappearance.json" << 'APPEARANCE_EOF'
{
  "cssSnippets": [
    "spinosa"
  ]
...
startup_prompt_text() {
  ...
  local prompt_template="${TEMPLATE_ROOT}/startup-prompt.md"
  local display_root
...
validate_workspace() {
  local path="$1"
  [[ -d "$path" ]] || return 1
  # Backward compat: check both new (.spinosa/) and old (.spinosa/) paths
  [[ -f "$path/.spinosa/workspace" ]] && return 0
  [[ -f "$path/.spinosa/workspace" ]] && return 0
  return 1
}
...
workspace_framework_version_value() {
  ...
  # Backward compat: try new path first, then old .spinosa/workspace
  local meta_file="${workspace_path}/.spinosa/workspace"
  [[ -f "$meta_file" ]] || meta_file="${workspace_path}/.spinosa/workspace"
  grep -m1 'framework_version:' "$meta_file" 2>/dev/null | awk '{print $2}'
}
...
require_workspace() {
  ...
  if [[ -f ".spinosa/workspace" || -f ".spinosa/workspace" ]]; then
...
# Move one legacy logs/ item to framework/logs/ — mv first, safe_copy fallback, then drop source.
...
archive_legacy_logs_memory_files() {
  ...
  for src in "${root}/framework/logs/session_metrics.tsv" "${root}/framework/logs/user_requests.md"; do
    [[ -f "$src" ]] || continue
    case "$src" in
...
# Migrate legacy workspace logs/ → hidden framework/logs/ (framework v0.7.6+).
migrate_workspace_logs_dir() {
  ...
  if [[ "$dry_run" -eq 1 ]]; then
    info "[dry-run] would migrate logs/ → framework/logs/"
    return 0
  fi

  timeout_sec="$(safe_copy_timeout_sec_for "$root")"
  spinosa_log INFO "migrate logs/ → framework/logs/ workspace=${root}"

  # Bulk rename only when framework/logs/ does not exist — otherwise mv would nest logs/ inside framework/logs/logs/.
  ...
  if [[ ! -e "$new" ]] && ! is_cloud_storage_path "$root"; then
    render_status_progress "migrating legacy logs" "logs/ → framework/logs/"
    if spinosa_run_with_timeout "$timeout_sec" mv "$old" "$new" 2>/dev/null; then
      clear_progress_line
      ok "Migrated logs/ → framework/logs/"
  ...
  for item in "$old"/*; do
    ...
    if migrate_legacy_logs_item "$item" "$new/$base" "$timeout_sec"; then
      migrated=$((migrated + 1))
      spinosa_log INFO "migrated logs/${base} → framework/logs/${base}"
    else
      clear_progress_line
      failed=$((failed + 1))
  ...
  if [[ "$failed" -eq 0 ]]; then
    ...
    if [[ -d "$old" ]]; then
      ...
      ok "Merged logs/ into framework/logs/ (${migrated} item(s))"
    fi
    return 0
  fi
