...
# ── package-release.sh — Build system release bundle ─────────────────────
# Usage: bash workspace-template/.bin/package-release.sh <version>
#
...
#   - checksums.txt
#
# The framework archive is built from workspace-template/.spinosa/release-files.tsv.
# install.sh is published as a separate release asset.

if [[ -z "${1:-}" ]]; then
  echo "Usage: bash workspace-template/.bin/package-release.sh <version>"
  echo "Example: bash workspace-template/.bin/package-release.sh 0.1.0"
  exit 1
fi
...
MANIFEST="${REPO_ROOT}/workspace-template/.spinosa/release-files.tsv"
...
# Also place the manifest at framework/spinosa/ so the installed CLI can find it
mkdir -p "${FRAMEWORK_DIR}/.spinosa"
cp "$MANIFEST" "${FRAMEWORK_DIR}/.spinosa/release-files.tsv"
...
if ! grep -q "^PINNED_VERSION=\"${VERSION}\"" "${DIST}/install.sh"; then
  echo "Error: staged install.sh PINNED_VERSION was not set to ${VERSION}"
  exit 1
fi
