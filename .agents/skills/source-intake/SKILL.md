...
2. Copy text-like files, native-readable files, and PDFs from the source location into `raw/`.
3. Skip `AGENTS.md` control files. They are repository instructions, not source evidence.
4. Do not create `.pointer.md` records for images, audio, or video. Leave those media files at the source location and include counts and processing gaps in the proposed log summary, `workspace_index.md`, or the relevant report.
...
- Images, audio, and video stay at the source location until a later processing pass creates text artifacts.
