...
description: |
  Fallback skill for the Spinosa serendippo sub-agent. Use when native vendor sub-agent dispatch is unavailable or when a Codex/OpenCode/Claude skill invocation is more appropriate than a separate sub-agent session.
---
<!-- generated by sync-agents from .agents/agents/spinosa-serendippo.md; edit the canonical agent or replace this file with a hand-written skill to opt out -->
