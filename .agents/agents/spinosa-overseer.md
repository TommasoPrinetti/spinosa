...
description: |
  ...
  agent-interception skill to read raw Codex/OpenCode session logs, builds
  file-access maps from actual tool usage, then cross-references against
  the corpus to identify untouched topics, stale evidence, and underutilized
  agents. Generates adversarial swarm recommendations for uncovered areas
  and delivers structured advisories to the orchestrator.
depends_on:
